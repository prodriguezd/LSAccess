...
// posici€ 0, 0.
	int i;
	Timer = TIMER_getTimer(); // Val m»s que n'hi hagi
...
void LcEnd(void) {
// El Destructor
	TIMER_closeTimer (Timer); // Ja no ens far? cap falta
}
...
void WaitForBusy(void) { char Busy;
	...
	do {
		...
		EnableDown();
		EnableDown();
		if (TIMER_getTics(Timer)) break; // M»s d'un ms vol dir quel LCD est? boig
	} while(Busy);
}
